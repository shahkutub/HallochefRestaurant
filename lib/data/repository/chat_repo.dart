...
class ChatRepo {
  ...
  //   return await apiClient.getData('${AppConstants.MESSAGE_LIST_URI}?${conversationID != null ? 'conversation_id' :userType == UserType.vendor ? 'vendor_id'
  //       : userType == UserType.customer ? 'customer_id' : 'delivery_man_id'}=${conversationID ?? userID}&offset=$offset&limit=10');
  // }

  Future<Response> getMessages(int offset, int userID, UserType userType, int conversationID) async {
    if(conversationID != null){
      return await apiClient.getData('${AppConstants.MESSAGE_LIST_URI}?conversation_id=${conversationID}&customer_id=${userID}&offset=1&limit=10');
    }else{
      return await apiClient.getData('${AppConstants.MESSAGE_LIST_URI}?customer_id=${userID}&offset=1&limit=10');

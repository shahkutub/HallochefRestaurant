...
import 'package:efood_multivendor_restaurant/controller/user_controller.dart';
...
class ChatController extends GetxController implements GetxService {
  ...
  Future<void> getConversationList(int offset) async {
    ...
    if(response.statusCode == 200) {
      if(offset == 1) {
        ...
        _conversationModel.conversation.addAll(ConversationsModel.fromJson(response.body).conversation);
      }
      int _index = -1;
      bool _sender;
      for(int index=0; index<_conversationModel.conversation.length; index++) {
        if(_conversationModel.conversation[index].receiverType == UserType.admin.name) {
          _index = index;
          _sender = false;
          break;
        }else if(_conversationModel.conversation[index].receiverType == UserType.admin.name) {
          _index = index;
          _sender = true;
          break;
        }
      ...
      if(_index != -1) {
        _hasAdmin = true;
        if(_sender) {
          _conversationModel.conversation[_index].sender = User(
          ...
          _conversationModel.conversation[_index].receiver = User(
            id: 0, fName: Get.find<SplashController>().configModel.businessName, lName: '',
            phone: Get.find<SplashController>().configModel.phone, email: Get.find<SplashController>().configModel.email,
            image: Get.find<SplashController>().configModel.logo,
          );
  ...
  Future<void> searchConversation(String name) async {
    ...
    if(response.statusCode == 200) {
      ...
      int _index = -1;
      bool _sender;
      for(int index=0; index<_searchConversationModel.conversation.length; index++) {
        if(_searchConversationModel.conversation[index].receiverType == UserType.admin.name) {
          _index = index;
          _sender = false;
          break;
        }else if(_searchConversationModel.conversation[index].receiverType == UserType.admin.name) {
          _index = index;
          _sender = true;
          break;
        }
      ...
      if(_index != -1) {
        if(_sender) {
          _searchConversationModel.conversation[_index].sender = User(
            id: 0, fName: Get.find<SplashController>().configModel.businessName, lName: '',
            phone: Get.find<SplashController>().configModel.phone, email: Get.find<SplashController>().configModel.email,
            image: Get.find<SplashController>().configModel.logo,
          );
        }else {
          _searchConversationModel.conversation[_index].receiver = User(
            id: 0, fName: Get.find<SplashController>().configModel.businessName, lName: '',
            phone: Get.find<SplashController>().configModel.phone, email: Get.find<SplashController>().configModel.email,
            image: Get.find<SplashController>().configModel.logo,
          );
  ...
  Future<void> getMessages(int offset, NotificationBody notificationBody, User user, int conversationID, {bool firstLoad = false}) async {
    ...
    //print('notificationBody'+notificationBody.customerId.toString());
    ...
    //   _response = await chatRepo.getMessages(offset, notificationBody.customerId, UserType.customer, conversationID);
    ...
    _response = await chatRepo.getMessages(offset, notificationBody.customerId, UserType.customer, conversationID);
     ...
     print('msgRes'+_response.body.toString());
    if (_response != null && _response.body['messages'] != {} && _response.statusCode == 200) {
      if (offset == 1) {
        ...
        if(conversationID != null && _conversationModel != null) {
          int _index = -1;
          for(int index=0; index<_conversationModel.conversation.length; index++) {
            if(conversationID == _conversationModel.conversation[index].id) {
              _index = index;
              break;
            }
          }
          if(_index != -1) {
            _conversationModel.conversation[_index].unreadMessageCount = 0;
          }
        }
        ...
        if(AppConstants.userInfoModel == null) {
          await Get.find<UserController>().getUserInfo();
        }
        /// Manage Receiver
        _messageModel = MessageModel.fromJson(_response.body);
        if(_messageModel.conversation == null) {
          _messageModel.conversation = Conversation(sender: User(
            id: AppConstants.userInfoModel.id, image: AppConstants.userInfoModel.image,
            fName: AppConstants.userInfoModel.fName, lName: AppConstants.userInfoModel.lName,
          ));
          //     receiver: notificationBody.restaurantId != null ? User(
          //   id: 0, fName: Get.find<SplashController>().configModel.businessName, lName: '',
          //   image: Get.find<SplashController>().configModel.logo,
          // ) : user);
        }
        ...
        //_sortMessageNew();
        _sortMessage(notificationBody.customerId);
      }else {
  ...
  Future<Response> sendMessage({@required String message, @required NotificationBody notificationBody,
    ...
    _response = await chatRepo.sendMessage(message, _myImages, notificationBody.customerId, UserType.customer, conversationID);
    ...
    if (_response.statusCode == 200) {
      ...
      if(_conversationModel != null && !_hasAdmin && (_messageModel.conversation.senderType == UserType.admin.name || _messageModel.conversation.receiverType == UserType.admin.name)) {
        _conversationModel.conversation.add(_messageModel.conversation);
        _hasAdmin = true;
      }
  ...
  void reloadConversationWithNotification(int conversationID) {
    ...
    for(int index=0; index<_conversationModel.conversation.length; index++) {
      if(_conversationModel.conversation[index].id == conversationID) {
        _index = index;
        _conversation = _conversationModel.conversation[index];
        break;
      }
    }
    if(_index != -1) {
      _conversationModel.conversation.removeAt(_index);
    }
    _conversation.unreadMessageCount++;
    _conversationModel.conversation.insert(0, _conversation);
    update();
  }
